...
class Player(wavelink.Player):
    def __init__(self, ctx, bot, guild_id: int, node: wavelink.Node):
        super(Player, self).__init__(ctx, bot, guild_id, node)
    ...
    async def player_loop(self):
        ...
        while True:
            ...
            # Invoke our controller if we aren't already.. this should happen really rarely
            if not self.update:
                await self.invoke_controller()
    ...
    async def invoke_controller(self, track: wavelink.Track = None):
        ...
        if track.is_stream:
            ...
            e.add_field(
                "Duration",
                f"{(datetime.timedelta(milliseconds=int(track.length)))}",
            )
...
class Music(commands.Cog):
    ...
    async def seek(self, ctx, *, position: str):
        """Move to a specified time in the song."""
    ...
    async def volume(self, ctx, *, volume: int):
        ...
        if not 0 <= volume <= 100:
            return await ctx.error("Please enter a value between 0 and 100.")
    ...
    async def queue(self, ctx):
        ...
        e = Embed(ctx, title=f"Upcoming - Next {len(upcoming)}", description=fmt)
    ...
    async def volume_up(self, ctx):
        ...
        await ctx.embed(f"Volume set to **{vol}%**!", 10)
        player.update = True
    ...
    async def volume_down(self, ctx):
        ...
        if vol < 0:
            vol = 0
            await ctx.embed("Player is currently muted", 10)
